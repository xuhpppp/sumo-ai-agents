...
## Bước 14 · Nối vào SimRunner, chạy full 1 giờ — trạng thái hiện tại: actuated-hybrid + coordination-context ✅

**Mục tiêu**: chế độ `llm` chạy trọn vẹn 1 giờ, sau đó tối ưu hiệu năng liên tục theo yêu cầu người dùng (nhiều vòng lặp — xem "Lịch sử rút gọn" bên dưới cho lý do từng bước).

**Kiến trúc hiện tại** (`src/sumo_agents/sim/runner.py`, mode `"llm"`):
- Round 1 (observe, Bước 12) chạy **song song** (`asyncio.gather`), rounds 2-4 (coalition/validate/approve, `agents/orchestrator.py`, Bước 13) chạy tuần tự — gộp thành **một** background task/chu kỳ (`_run_llm_decision_cycle`), không `await`. Chu kỳ trước chưa xong khi tới chu kỳ sau → **bỏ qua**, đếm vào `runs.summary.n_skipped_cycles`.
- `llm` mode nạp mạng **actuated thật** (`sim_actuated.sumocfg`/`net_actuated.xml`, cả 36 nút actuated — không chỉ 6 nút có agent) — SUMO tự actuate MỖI BƯỚC MÔ PHỎNG qua vòng cảm ứng ở vạch dừng. LLM chỉ đặt biên chiến lược `SetGreenBounds(junction_id, phase_id, min_green_s, max_green_s)` mỗi chu kỳ — action `AdjustPhaseSplit`/`SetCycleLength`/`SetOffset` bị loại khỏi `ActionUnion` LLM thấy (vẫn tồn tại trong `validator.Action` cho caller khác); `request_vms` vẫn `NotImplementedError` có chủ đích, đúng phạm vi Bước 19.
- `JunctionAgent.observe()` nhận `history` (3 chu kỳ gần nhất, cũ→mới) + `per_phase` (queue/wait phân rã theo `phase_id`, từ `phase_lane_groups()`/`per_phase_traffic()` trong `sim/actuators.py`) — cả hai cùng thuộc `user` prompt dựng lại mỗi chu kỳ.
- Coalition round (2, `JunctionAgent.reply()`) + `SupervisorAgent.review()` (vòng 4) có thêm `NeighborLink(neighbor_id, distance_m, travel_time_s)` — khoảng cách/travel-time free-flow tới hàng xóm kề, từ `agents/topology.py`'s `neighbor_links()` — để đánh giá mức khẩn cấp phối hợp (10-15s = xử lý sớm, 60s+ = thường tự giải quyết).
- `LLM_REALTIME_SPEEDUP = 5.0` ghìm tốc độ mô phỏng **chỉ cho mode `llm`** (baseline không ghìm) để 1 chu kỳ LLM (10-20s thực) có cơ hội xong trước chu kỳ 90s-mô-phỏng sau.
- `decisions.effect` (before/after) điền ở chu kỳ kế; `runs.summary` có `total_llm_cost_usd`/`n_llm_calls`/`n_skipped_cycles`.

**Test**: 117/117 pass toàn suite (`test_topology`/`test_junction`/`test_supervisor`/`test_orchestrator`/`test_validator`/...).

**Quyết định thiết kế cần nhớ khi tiếp tục tối ưu**:
- **Vì sao actuated-hybrid thay vì tiếp tục tinh chỉnh retiming định kỳ**: 3 lần chạy full liên tiếp trên action space cũ (base → +history → +per-phase) đều bị chặn trần dưới `actuated` (107.40s), lần cuối còn tệ đi khi agent hành động nhiều hơn. Root cause xác nhận qua so sánh `net.xml` vs `net_actuated.xml`: khác biệt là **độ mịn thời gian phản ứng** (SUMO actuate mỗi bước mô phỏng vs LLM mỗi 90s), không phải chất lượng ra quyết định — kể cả retiming định kỳ hoàn hảo cũng bị chặn trần bởi cấu trúc này.
- **Vì sao KHÔNG hồi sinh `set_offset` cho coordination**: actuated có chu kỳ biến thiên theo cảm biến (xác nhận qua probe thủ công), nên đồng bộ pha kiểu làn sóng xanh cổ điển sẽ trôi lệch dần theo thời gian — rủi ro cao, lợi ích không chắc. Chọn bổ sung ngữ cảnh khoảng cách/travel-time cho lớp phối hợp sẵn có thay vì thêm cơ chế mới.
- **Vì sao bỏ direction (upstream/downstream) tĩnh trong `NeighborLink`**: `grid_4x4` có đường 2 chiều giữa mọi cặp nút kề (`test_neighbor_relationship_is_symmetric`), không có dòng chảy ổn định để gán nhãn chiều — chỉ khoảng cách/travel-time là xác định rõ ràng.
- **actuated-hybrid không phải liều thuốc vạn năng**: khi tắc nghẽn quá nặng (teleport xe do chờ quá lâu), nâng biên không giúp gì — pha xanh co về `minDur` vì vòng cảm ứng gần như không thấy xe di chuyển qua để kích hoạt kéo dài. Chỉ hiệu quả khi giao thông còn "chảy".

**Kết quả full run — tiến trình qua từng vòng lặp** (`grid_4x4`, seed=42):

| Vòng lặp | run_id | mean_travel_time_s | so với fixed | khoảng cách còn lại tới actuated (107.40s) | n_skipped_cycles | Ghi chú |
|---|---|---|---|---|---|---|
| `fixed` (baseline) | acb816a4 | 134.35 | — | +26.95 | — | không làm gì |
| llm v1 (observe cơ bản) | 561c302f | 134.35 | 0% | +26.95 | 0/42 | 0 tin nhắn coalition — hoàn toàn thụ động, y hệt `fixed` |
| llm +history | 81760b55 | 137.54 | +2.4% (tệ hơn) | +30.14 | 0/43 | phá được thụ động, nhưng hành động chưa giúp gì |
| llm +per-phase (gap 2) | 932194c2 | 148.18 | +10.3% (tệ hơn) | +40.78 | 11/44 | chọn đúng phase_id hơn nhưng hành động+phối hợp nhiều hơn → skip nhiều, mạng tệ đi |
| llm **actuated-hybrid** | 37942b93 | 118.31 | −11.9% | +10.91 | 5/~44 | bước ngoặt: đổi cơ chế (biên chiến lược) thay vì tinh chỉnh dữ liệu đầu vào |
| llm actuated-hybrid **+coordination-context** | 51a9788c | **111.85** | −16.8% | **+4.45** | 3/~44 | hiện tại tốt nhất — 83.5% quãng đường fixed→actuated |

**Mẫu hình xuyên suốt cả quá trình**: chất lượng quyết định (ít nhiễu, ít dao động) quan trọng hơn tần suất hành động — coordination-context hành động ÍT hơn actuated-hybrid thuần (210 `no_action`/24 `set_green_bounds` so với 193/29) mà kết quả vẫn tốt hơn.

**Đối chiếu chỉ số khác ngoài travel time** (run actuated-hybrid `37942b93`, mẫu `Metric` mỗi 10s toàn mạng 36 nút, so 4 mode cùng seed=42; chưa đo lại riêng cho run coordination-context):

| Chỉ số | fixed | actuated (trần) | maxpressure | llm actuated-hybrid | % khoảng cách fixed→actuated đã thu hẹp |
|---|---|---|---|---|---|
| mean_travel_time_s | 134.35 | 107.40 | 139.51 | 118.31 | 59.5% |
| avg mean_waiting_s | 6.49 | 2.81 | 6.23 | 3.20 | 89.5% |
| avg queue_len | 4.55 | 2.32 | 4.89 | 3.10 | 64.7% |
| avg mean_speed (m/s) | 9.36 | 11.00 | 9.29 | 10.76 | 85.3% |
| tổng CO2 (~kg, ước lượng) | 108.1 | 88.1 | 111.9 | 95.8 | 61.7% |

`mean_waiting_s`/tốc độ trung bình tiến sát `actuated` nhất (~85-90%); `queue_len`/CO2 còn khoảng cách lớn nhất (~35-38%) — nhạy với đúng những khoảnh khắc tắc cục bộ mà các chu kỳ bị skip có thể góp phần.

**Bằng chứng coordination-context thực sự được dùng, không phải cải thiện ngẫu nhiên** (run `51a9788c`): trong 85 tin nhắn coalition, có 2 tin `object` trích dẫn thẳng số liệu khoảng cách mới — *"Khoảng cách chỉ 179 m (13 giây) nên tôi sẽ theo dõi, nhưng không đồng ý can thiệp..."* (C2 phản đối B2) và một trường hợp B2 phát hiện đề xuất của B1 vi phạm cấu trúc trong khi vẫn viện dẫn khoảng cách để giải thích mức độ theo dõi. `supervisor_verdict`: 232 `approved`/0 `modified`/0 `denied` lần này — mục "Corridor adjacency" có dữ liệu nhưng chưa dẫn tới can thiệp `modified` nào ở run này. Độ trễ LLM tăng nhẹ do prompt dài hơn (supervisor mean=6.6s/p95=13.0s/max=16.1s, trước 6.4s/8.8s/9.9s) nhưng `n_skipped_cycles` vẫn giảm (3 so với 5) nhờ tổng số coalition message ít hơn (85 so với 123) bù lại.

**Kết luận hiện tại**: vẫn chưa vượt `actuated` thuần (còn cách 4.45s) nhưng khoảng cách đã rất nhỏ. Hướng đào sâu tiếp nếu muốn: xem `supervisor_verdict` có bao giờ `modified` dựa trên corridor qua nhiều seed hơn, hoặc mở rộng phạm vi nút agent để corridor-context có nhiều cặp candidate hơn.

**Lịch sử rút gọn** (chi tiết đầy đủ lỗi/debug/test/kiểm chứng từng bước — xem `git log`/git blame trên các file liên quan hoặc hỏi lại nếu cần):
- **v1 → +history**: `observe()` ban đầu chỉ thấy snapshot hiện tại, không có xu hướng qua các chu kỳ → luôn `no_action` dù có tắc thật trong cửa sổ sự cố (1200-1800s) → thêm `history` 3 chu kỳ gần nhất vào prompt.
- **+history → +per-phase (gap 2)**: `queue_len`/`mean_waiting_s` tổng gộp cả nút không đủ để chọn đúng `phase_id` → thêm phân rã theo pha.
- **+per-phase → actuated-hybrid**: agent chọn đúng `phase_id` hơn nhưng hành động+phối hợp nhiều hơn lại làm mạng tệ đi (skip tăng do latency cộng dồn 3 tầng round1→round2→supervisor) → nhận ra root cause thật là tần suất actuate (90s LLM vs mỗi bước mô phỏng của SUMO actuated), không phải chất lượng dữ liệu đầu vào → chuyển hẳn action space sang `SetGreenBounds` trên nền `net_actuated.xml`.
- **actuated-hybrid → coordination-context**: supervisor có quyền `modified` một đề xuất nhưng thiếu dữ liệu khoảng cách/travel-time để suy luận mức khẩn cấp giữa các nút → thêm `NeighborLink`.
...
> Nhắc lại từ plan §7: `llm` **có thể thua** `actuated`. Sau nhiều vòng lặp (xem bảng tiến trình ở trên), `llm` actuated-hybrid + coordination-context hiện đã vượt xa `fixed`/`maxpressure` và chỉ còn cách `actuated` 4.45s (111.85s so với 107.40s) — chưa thắng nhưng khoảng cách rất nhỏ. Tuỳ người dùng quyết định: tiếp tục đào sâu hướng phối hợp/corridor, hay chấp nhận kết quả hiện tại và sang Phase 3 (dashboard).
...
**Mục tiêu**: REST phục vụ lịch sử run, WebSocket đẩy sự kiện live — không đụng `sim/runner.py`/`obs/store.py`.

**Đã làm** (`src/sumo_agents/web/app.py`):
- **Cơ chế live**: KHÔNG sửa `sim/runner.py`/`Store` để thêm pub-sub — dashboard chỉ là một reader khác trên cùng Postgres. WebSocket loop mở **1 session mới mỗi lần poll** (mặc định 1s/lần, `POLL_INTERVAL_S`), không giữ 1 session sống suốt kết nối — cần thiết để mỗi lần đọc thấy đúng commit mới nhất từ tiến trình `sim` khác (READ COMMITTED của Postgres đảm bảo điều này chỉ khi transaction là mới). Con trỏ theo dõi: `id` lớn nhất đã thấy cho `messages`/`decisions`/`llm_calls` (khởi tạo = max hiện tại lúc connect, nên chỉ đẩy dữ liệu MỚI sau khi connect — lịch sử là việc của REST), `sim_time` lớn nhất cho `metrics` (không có cột `id` tự tăng).
- **REST**: `GET /runs` (lọc theo `mode`, mới nhất trước), `GET /runs/{id}` (404 nếu không có), `GET /runs/{id}/messages|decisions|llm_calls` (phân trang qua `since_id`), `GET /runs/{id}/metrics` (qua `since_sim_time`) — cùng 1 hàm truy vấn `since_id`/`since_sim_time` dùng chung cho cả REST lẫn WebSocket polling, không viết 2 lần. Thêm `/runs/{id}/llm_calls` dù dòng tóm tắt Bước 15 trong bảng tiến độ chỉ nêu `messages|decisions|metrics` — cần cho panel Monitoring (Bước 16: token/chi phí/cache hit rate/latency, đều nằm ở bảng `llm_calls`).
- **WebSocket** `/ws/runs/{run_id}`: kiểm tra run tồn tại **trước** `accept()` (đóng sạch với code 4404 nếu không có, không chấp nhận rồi mới báo lỗi); mỗi lần poll gửi tối đa 4 loại sự kiện gộp theo batch (`messages`/`decisions`/`llm_calls`/`metrics`, mỗi loại 1 message JSON `{"type", "data": [...]}` nếu có dòng mới) rồi `asyncio.sleep`; khi `runs.finished_at` đã set VÀ không còn dòng mới nào ở lần poll đó → gửi `run_finished` kèm bản ghi run đầy đủ rồi tự đóng socket (không poll một run đã xong mãi mãi).
- CORS mở (`allow_origins=["*"]`) — dashboard là POC chạy local, frontend (Bước 16) sẽ là dev server cổng khác (kiến trúc trong plan tách FastAPI/WebSocket khỏi "Web dashboard" thành 2 khối riêng).
- `python -m sumo_agents.web.app` chạy uvicorn trực tiếp (đọc `DASHBOARD_HOST`/`DASHBOARD_PORT`, mặc định `127.0.0.1:8000`) — hoặc `uvicorn sumo_agents.web.app:app --reload`.

**Test**: `tests/test_web.py` (13 test mới, `FastAPI TestClient` + ghi đè `get_session_factory` sang SQLite in-memory có sẵn từ Bước 4) — REST trả đúng dữ liệu/404, `since_id`/`since_sim_time` chỉ trả dòng mới, WebSocket từ chối `run_id` lạ (code 4404), chỉ đẩy dòng ghi SAU khi connect (không phải lịch sử), đẩy đúng cả `messages` lẫn `decisions`, gửi `run_finished` khi `finish_run()` được gọi. `POLL_INTERVAL_S` monkeypatch xuống 0.02s để test nhanh (đọc lại tên module-level mỗi vòng lặp, không phải giá trị chụp lúc import, nên gán lại từ test có tác dụng ngay). Toàn suite: **130/130 pass**.

**Kiểm chứng thật** (`scripts/verify_web_dashboard.py`, $0, không SUMO/không OpenAI): baseline (`fixed`/`actuated`) chạy hết 1 giờ mô phỏng chỉ tốn ~3s thực — quá nhanh để gắn WebSocket client giữa chừng một cách đáng tin cậy — nên thay vì chạy sim thật, script khởi động uvicorn thật (thread nền) + kết nối `websockets` client thật tới `/ws/runs/<run_id>` + ghi trực tiếp qua đúng class `Store` mà `sim/runner.py` dùng (có độ trễ nhỏ giữa các lần ghi, mô phỏng "sim đang ghi khi client đang xem"), tất cả trên Postgres thật (container `sumo-ai-agents-db-1`):
- REST: `GET /runs` thấy run mới, `GET /runs/{id}` đúng dữ liệu, id lạ → 404.
- WebSocket: nhận đúng sự kiện `messages` (~trong 5s sau khi ghi), `decisions`, `metrics` theo đúng thứ tự ghi; sau `store.finish_run()` nhận `run_finished` rồi server tự đóng socket sạch (không cần client đóng).
- Dọn sạch: xoá đúng 1 row run test khỏi Postgres ở cuối script (`finally`), xác nhận lại bằng truy vấn riêng — 0 row còn sót.
- **Lưu ý minh bạch**: đây là kiểm chứng cơ chế thật (Postgres thật, FastAPI/uvicorn thật, WebSocket thật) chứ KHÔNG phải xem một sim thật đang chạy — vì lý do chi phí (chỉ `--mode llm` mới đủ chậm về thời gian thực để `wscat` kịp gắn vào giữa chừng, và mode đó tốn tiền OpenAI thật). Script tự in sẵn 3 lệnh (`uvicorn ...`, `python -m sumo_agents.sim.runner --mode llm ...`, `wscat -c ws://...`) để người dùng tự làm đúng như câu chữ DoD nếu muốn xem tận mắt một run thật.

**DoD đạt được**: cơ chế realtime đã kiểm chứng đúng thật (Postgres + FastAPI + WebSocket thật, không mock) — người dùng tự chạy `wscat` cạnh một `--mode llm` thật khi muốn xác nhận trên đúng câu chữ DoD ("khi sim đang chạy"), theo quy ước "người dùng tự chạy sim tốn tiền" xuyên suốt project này.

## Bước 16 · 4 panel frontend ✅

**Mục tiêu**: người ngoài xem dashboard hiểu được chuyện gì đang xảy ra mà không cần giải thích code.

**Đã làm**: `src/sumo_agents/web/static/{index.html,app.js,style.css}` — vanilla JS/HTML/CSS, KHÔNG build tool/framework (khớp đúng cấu trúc `web/{app.py,static/}` trong plan mục 8) — `app.py` phục vụ trực tiếp qua `StaticFiles(html=True)`, mount **sau cùng** (sau mọi route REST/WebSocket của Bước 15) để `/runs`, `/health`,... không bị route tĩnh nuốt mất.
- **Network snapshot** (thay "Live map"): plan tự nói rõ Phase 3 tạm dùng `sumo-gui`, chưa vẽ canvas theo vị trí xe thật (không có dữ liệu toạ độ xe được expose qua API) — thay bằng lưới trạng thái nút, tô màu theo `queue_len` mới nhất (thang màu sequential 1-hue xanh dương, 5 bậc, theo đúng "compare magnitude on a grid → heatmap, sequential hue" của dataviz skill), có ghi chú rõ cách xem bản đồ hình học thật (`--gui`).
- **Agent chat**: timeline `messages` nhóm theo `cycle_id` (divider khi đổi cycle), badge `intent` màu riêng, hiện `rationale` — build bằng `el()`/`textContent`, không dùng `innerHTML` (rationale là text do LLM sinh ra, cùng nguyên tắc "untrusted data" đã áp dụng cho prompt ở `agents/junction.py`).
- **Decision log**: bảng `decisions`, mới nhất trên cùng, badge màu theo `validator_status`/`supervisor_verdict` dùng đúng status palette cố định của dataviz skill (good/warning/critical), luôn kèm icon+chữ (không chỉ màu).
- **Monitoring**: KPI row 5 stat tile (tổng chi phí, cache hit rate, latency p50/p95, tỉ lệ reject validator, số chu kỳ skip) — tính trực tiếp từ `llm_calls`/`decisions` đã tải, không phải chart (đúng "a handful of headline numbers → stat tile" của dataviz skill). `n_skipped_cycles` cố tình hiện "—" cho tới khi run kết thúc — chu kỳ bị skip không bao giờ ghi row `decisions` (đã xác nhận ở Bước 14 follow-up), nên không thể suy ra khi run còn đang chạy.
- **Live**: mở 1 WebSocket/run (Bước 15), cập nhật đúng panel liên quan theo `event.type` (`messages`/`decisions`/`llm_calls`/`metrics`/`run_finished`); dropdown chọn run tự refresh danh sách mỗi 10s (không ngắt view đang xem); lịch sử tải qua vòng lặp phân trang `since_id`/`since_sim_time` (cần thiết vì `metrics` một run 1 giờ có thể tới ~13.000 dòng, vượt xa giới hạn 1 trang mặc định của Bước 15).
- **Màu sắc**: copy nguyên văn bảng màu tham chiếu đã validate của dataviz skill (không tự bịa hex mới) — light/dark qua CSS custom properties (`prefers-color-scheme` + `data-theme` override).

**Kiểm chứng thật**: chạy server thật (`python -m sumo_agents.web.app`) trên Postgres thật, seed dữ liệu thực tế (messages nhiều cycle/intent, 1 decision `ok`+`effect`, 1 decision `rejected`, 2 llm_calls có cost/latency, metrics nhiều nút) qua đúng `Store`, rồi `curl` từng REST endpoint + mở 1 WebSocket client thật — xác nhận từng field JSON khớp chính xác với đúng tên field `app.js` đọc (`sender`/`recipients`/`intent`/`rationale`, `validator_status`/`supervisor_verdict`/`effect`, `cost_usd`/`cached_tokens`/`latency_ms`, `queue_len`/`mean_waiting_s`), và sự kiện WebSocket đúng dạng `{"type", "data"}` cho cả 3 loại. Dọn sạch run test khỏi Postgres sau khi xong, xác nhận lại — 0 row còn sót. 130/130 test cũ vẫn pass (không có test mới — Bước 16 là frontend tĩnh, không có logic Python để unit-test; đã kiểm chứng qua dữ liệu thật ở trên thay thế).
- **Giới hạn minh bạch**: môi trường này không có trình duyệt headless (không có `chromium-cli`/Playwright cài sẵn) nên KHÔNG tự chụp được ảnh màn hình để xác nhận trực quan (màu sắc hiển thị đúng, layout không vỡ, không có lỗi console JS). Đã xác nhận đầy đủ ở mức dữ liệu/API (đúng field, đúng thứ tự, đúng sự kiện live) — phần hiển thị trực quan cần người dùng tự mở `http://127.0.0.1:8000/` (sau khi chạy `python -m sumo_agents.web.app`) để xác nhận lần cuối, việc này chỉ mất vài giây và không đáng để cài thêm một bộ công cụ trình duyệt headless mới cho riêng việc này.
