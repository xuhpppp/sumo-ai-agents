...
**Đối chiếu các chỉ số khác ngoài `mean_travel_time_s`** (từ bảng `metrics`, lấy mẫu mỗi 10s mô phỏng trên cả 36 nút, cùng seed=42, so 4 run: `fixed`=`acb816a4`, `actuated`=`a58de666`, `maxpressure`=`f778ca87`, `llm` actuated-hybrid=`37942b93`) — cải thiện đồng bộ trên mọi chỉ số, không chỉ travel time:
...
| thời gian mô phỏng hết 4500 chuyến (s) | 3811 | 3751 | 3811 | 3751 | khớp đúng `actuated` |

`mean_waiting_s`/tốc độ trung bình tiến sát `actuated` nhất (~85-90% khoảng cách); `queue_len`/CO2 còn khoảng cách lớn nhất (~35-38%), hợp lý vì đây là 2 chỉ số tích lũy nhạy với đúng những khoảnh khắc tắc cục bộ mà 5 chu kỳ bị skip có thể góp phần.

## Follow-up coordination-context (thêm khoảng cách/travel-time thực vào coalition + supervisor)

Người dùng muốn tiếp tục cải thiện `llm` actuated-hybrid với kỳ vọng có thể vượt qua `actuated` (107.40s), chấp nhận chi phí tăng nhẹ. Đọc lại `orchestrator.py`/`supervisor.py`/`topology.py` phát hiện: lớp phối hợp (coalition round + `SupervisorAgent`) đã tồn tại sẵn về kiến trúc — supervisor nhận TẤT CẢ đề xuất + tin nhắn coalition cùng lúc, có quyền `modified` một đề xuất — nhưng **mù thông tin quan trọng nhất để phối hợp thật**: `signalized_neighbor_map` chỉ là danh sách kề nhau, không có khoảng cách/thời gian di chuyển, nên supervisor không thể suy luận kiểu "thay đổi ở nút A bao lâu nữa thì ảnh hưởng tới nút B". Ngoài ra prompt supervisor còn ví dụ xung đột bằng `set_offset`/`adjust_phase_split` — 2 action đã bị khai tử ở follow-up actuated-hybrid, không còn khớp action space thật.

**Hướng đã chọn** (KHÔNG hồi sinh `set_offset`): actuated có chu kỳ biến thiên theo cảm biến (đã xác nhận qua probe trước đó), nên đồng bộ pha kiểu làn sóng xanh cổ điển sẽ trôi lệch dần — rủi ro cao, lợi ích không chắc. Thay vào đó, giữ nguyên action space `set_green_bounds` đã kiểm chứng an toàn, chỉ bổ sung NGỮ CẢNH khoảng cách/travel-time để lớp phối hợp hiện có ra quyết định thông minh hơn.

Direction (upstream/downstream tĩnh) bị bỏ có chủ đích: `grid_4x4` có đường 2 chiều giữa mọi cặp nút kề nhau (`test_neighbor_relationship_is_symmetric`), nên một nhãn chiều tĩnh sẽ không ứng với dòng chảy thật nào ổn định — chỉ khoảng cách/thời gian di chuyển là xác định rõ ràng bất kể chuyển động thực tế dùng làn nào.

**Đã làm**:
- **`agents/topology.py`**: `NeighborLink(neighbor_id, distance_m, travel_time_s)` + `neighbor_links(net_file) -> dict[str, dict[str, NeighborLink]]` — quãng đường/thời gian di chuyển tự do (free-flow) tới từng hàng xóm kề trực tiếp, đọc thẳng từ `net.xml` (cùng nguồn dữ liệu `signalized_neighbor_map` đã dùng, không thêm phụ thuộc mới). Khi có nhiều cạnh nối cùng một cặp, giữ cạnh ngắn hơn.
- **`agents/junction.py`**: `reply()` nhận thêm `link: NeighborLink | None` — `_build_reply_user_prompt` thêm dòng `corridor_to_sender: distance_m=... free_flow_travel_time_s=...`. Hệ thống prompt (đoạn "Coalition round") giải thích cách dùng con số này để đánh giá mức khẩn cấp. Cache_key bump `v6`→`v7`.
- **`agents/supervisor.py`**: `review()` nhận thêm `links` — `_build_user_prompt` thêm mục "Corridor adjacency between today's candidates", chỉ hiện cặp mà CẢ HAI bên đều là candidate chu kỳ này (không phải toàn bộ neighbor_links tĩnh). System prompt: thay ví dụ xung đột cũ (`set_offset`/`adjust_phase_split`) bằng ví dụ đúng action space thật (`set_green_bounds` siết cùng một hành lang theo 2 hướng ngược nhau), thêm hướng dẫn dùng travel_time để đánh giá mức khẩn cấp (một xung đột cách nhau 10-15s cần xử lý sớm hơn một xung đột cách 60s+). Cache_key bump `v1`→`v2`.
- **`agents/orchestrator.py`**: `run_decision_cycle`/`_run_coalition_round` nhận thêm `neighbor_links` (mặc định `None`/rỗng — không phá vỡ caller cũ không có dữ liệu topology), truyền đúng `link` (theo góc nhìn của RECIPIENT, không phải sender) vào từng lệnh `reply()`, và truyền toàn bộ map vào `supervisor.review()`.
- **`sim/runner.py`**: tính `llm_neighbor_links = neighbor_links(net_actuated.xml)` một lần khi khởi động (topology không đổi trong run, giống `llm_neighbor_map`), truyền vào mỗi chu kỳ quyết định.
- **12 test mới** (`test_topology.py` +4, `test_junction.py` +3, `test_supervisor.py` +4, `test_orchestrator.py` +2 -1 sửa lại `_FakeAgent`/`_FakeSupervisor` cho tham số mới): khoảng cách/travel-time đúng theo `net.xml` thật (grid đều, 179.2m/13.89m/s), đối xứng, đúng dữ liệu tới đúng người nhận (không lẫn giữa 2 hàng xóm), mục corridor chỉ hiện cặp candidate-candidate, callback không có `neighbor_links` vẫn chạy được (link=None khắp nơi, không crash). 117/117 test pass.
- **Kiểm chứng thật** (`scripts/verify_llm_mode.py`): check 1e mới ($0, không TraCI/LLM) — `neighbor_links` khớp đúng tập hàng xóm của `signalized_neighbor_map`, xác nhận có ít nhất 1 cặp AGENT-AGENT kề nhau thật (14 cặp có hướng trong khối 2x3 `B0,B1,B2,C0,C1,C2`) để mục corridor của supervisor có dữ liệu thật để hiện. Check 2 (2 chu kỳ LLM thật, ~$0.1) chạy trọn vẹn không crash — lưu ý minh bạch: cả 2 chu kỳ đều `no_action` (giao thông còn nhẹ ở đầu run, giống mọi lần smoke-check trước), nên coalition round chưa thực sự kích hoạt trong lần chạy ngắn này — đường dẫn corridor-context được xác nhận đúng qua unit test (đường đi rõ ràng, xác định), chưa qua một message thật có nội dung corridor trong smoke check.

**Run full 1 giờ với coordination-context** (`run_id=51a9788c-4034-4503-b283-0127e988b646`, seed 42, 2026-09-15): **cải thiện tiếp, gần chạm trần `actuated` hơn bao giờ hết**.
- `mean_travel_time_s=111.85s` — so với actuated-hybrid trước đó (118.31s): thêm **−6.46s**. So với trần `actuated`=107.40s, khoảng cách còn lại chỉ **4.45s** (từ 10.9s xuống 4.45s, thu hẹp thêm ~59% phần còn lại). Tính trên toàn thang từ `fixed`(134.35s) tới `actuated`(107.40s), run này đã đi được **83.5%** quãng đường.
- **3/~44 chu kỳ bị skip** (giảm tiếp từ 5). 234 decision rows = 39 chu kỳ hoàn thành × 6 nút. `action_type`: 210 `no_action` / 24 `set_green_bounds` — ÍT hành động hơn cả actuated-hybrid thuần (193/29) mà kết quả vẫn tốt hơn, củng cố mẫu hình xuyên suốt Step 14: chất lượng quyết định (ít nhiễu, ít dao động) quan trọng hơn tần suất hành động.
- **Bằng chứng trực tiếp cho thấy corridor-context thực sự được dùng**: trong 85 tin nhắn coalition (giảm từ 123, do ít đề xuất hơn), có 2 tin `object` — cả hai đều trích dẫn thẳng con số khoảng cách mới: *"Khoảng cách chỉ 179 m (13 giây) nên tôi sẽ theo dõi, nhưng không đồng ý can thiệp..."* (C2 phản đối B2) và một trường hợp B2 phát hiện đúng đề xuất của B1 vi phạm cấu trúc (`min_green_s=52 > max_green_s=45`) trong khi vẫn viện dẫn khoảng cách 179m/13s để giải thích mức độ theo dõi. Đây là bằng chứng thực nghiệm rằng model KHÔNG bỏ qua ngữ cảnh mới mà chủ động dùng nó để cân nhắc mức khẩn cấp, đúng như thiết kế.
- **`supervisor_verdict`: 232 `approved` / 0 `modified` / 0 `denied`** — mục "Corridor adjacency" trong prompt supervisor được cấp dữ liệu nhưng KHÔNG dẫn tới can thiệp `modified` nào ở lần chạy này (khác với object ở round 2, đây là round 4); có thể vì round 2 (coalition) đã tự giải quyết phần lớn xung đột tiềm ẩn trước khi tới supervisor, hoặc đơn giản là chu kỳ này chưa có xung đột đủ nghiêm trọng để supervisor cần sửa.
- **Độ trễ supervisor tăng nhẹ**: mean=6.6s, p95=13.0s, max=16.1s (trước: mean=6.4s/p95=8.8s/max=9.9s) — hợp lý vì prompt dài hơn (thêm mục corridor); dù vậy `n_skipped_cycles` vẫn giảm (3 so với 5), có lẽ vì tổng số coalition message ít hơn (85 so với 123) bù lại.
- **Kết luận**: hướng coordination-context đúng như kỳ vọng — đóng góp cải thiện thực, có bằng chứng model thực sự dùng dữ liệu mới chứ không phải cải thiện ngẫu nhiên. Vẫn chưa vượt qua `actuated` thuần (4.45s), nhưng khoảng cách đã rất nhỏ. Hướng tiếp theo nếu muốn đào sâu hơn: xem `supervisor_verdict` có bao giờ `modified` dựa trên corridor không qua nhiều seed hơn, hoặc mở rộng phạm vi nút agent để corridor-context có nhiều cặp candidate hơn để phát huy tác dụng.
